...
// === НАСТРОЙКИ (БЕРУТСЯ АВТОМАТОМ ИЗ ФАЙЛА .env) ===
...
// =====================================================================
// === ЛОГИКА БОТА (Маскарад, а не вымогательство) ===
// =====================================================================
...
bot.start(async (ctx) => {
  ...
  if (payload && payload.startsWith('w_')) {
    ...
    if (!targetUser) return ctx.reply('Этот человек еще не на нашем балу маскарада :(');
    
    ctx.session = { targetId: targetId, sender_step: 'ask_name' };
    await ctx.reply(
      `🎭 <b>Оставь тайное послание для ${targetUser.first_name}</b>\n\n` +
      `Напиши свое имя и прикрепи фото. Твоя личность останется в секрете за маской, пока получатель сам не решит её снять!\n\n` +
    ...
    await ctx.reply(
      `👋 <b>Добро пожаловать на бал маскарад, ${badge} ${ctx.from.first_name}!</b>\n\n` +
      `Это Шёпот — место, где рождаются тайны.\n\n` +
      `📱 Кидай свою ссылку в соцсети, и люди будут писать тебе анонимные признания.\n` +
      `🎭 Все отправители скрыты за масками. Хочешь узнать, кто стоит за маской? В Магазине есть ключи к разгадке...`,
...
bot.hears('🔗 Моя ссылка', (ctx) => {
  ...
  ctx.reply(`🔗 <b>Твоя ссылка для соцсетей:</b>\n\n<code>${link}</code>\n\nКидай в Instagram, TikTok или VK, чтобы получать анонимки!`, { parse_mode: 'HTML' });
});

bot.hears('📨 Мои сообщения', (ctx) => {
  const msgs = getUnreadMessages(ctx.from.id);
  if (msgs.length === 0) return ctx.reply('📭 Тишина... Поделись ссылкой, чтобы призвать послания!');
...
bot.hears('🎭 Магазин', (ctx) => {
  ctx.reply(
    `🎭 <b>Снять маску</b>\n\nЗа каждым анонимным посланием скрывается человек. Хочешь узнать, кто он?`,
    { parse_mode: 'HTML', ...Markup.inlineKeyboard([
      [Markup.button.callback('🎭 Снять маску с 1 автора (149 ₽)', 'shop_reveal')],
      [Markup.button.callback('💎 VIP — 5 снятий масок (399 ₽)', 'shop_vip')],
      [Markup.button.callback('👑 PREMIUM — Всевидящее око (799 ₽)', 'shop_premium')]
    ])}
  );
});
...
bot.hears('❓ Помощь', (ctx) => {
  ctx.reply(
    ...
    `1. Кидай ссылку в соцсети.\n` +
    `2. Люди пишут тебе анонимные послания (текст ты читаешь бесплатно).\n` +
    `3. Все авторы в масках 🎭. Если хочешь узнать, кто именно тебе написал — в Магазине можно купить "Снятие маски".`,
    { parse_mode: 'HTML' }
  );
});
...
bot.on('photo', async (ctx) => {
  ...
  await ctx.reply('✅ Фото получено! Оно будет скрыто за маской.\n\n📝 Теперь напиши своё послание (текст):');
});
...
bot.on('text', async (ctx) => {
  ...
  if (ctx.session.sender_step === 'ask_name') {
    ...
    return ctx.reply('📸 Отлично! Теперь отправь <b>своё реальное фото</b> (как картинку). Оно будет скрыто маской!', { parse_mode: 'HTML' });
  }
  ...
  if (ctx.session.sender_step === 'ask_message') {
    ...
    await ctx.reply('✅ Послание доставлено! Твоя маска на месте 🎭', getMainMenu());
    try {
      await bot.telegram.sendMessage(targetId, '🤫 <b>Тебе пришло новое послание от незнакомца!</b>', {
        parse_mode: 'HTML',
...
bot.action('read_messages', (ctx) => {
  const msgs = getUnreadMessages(ctx.from.id);
  if (msgs.length === 0) return ctx.answerCbQuery('Посланий нет!');
  ctx.session = ctx.session || {};
  ctx.session.msgQueue = msgs.map(m => m.id);
  showNextMessage(ctx);
});
...
bot.action(/^read_(.+)$/, (ctx) => {
  ...
  if (hasAccess) {
    ...
    ctx.answerCbQuery();
    ctx.reply(
      `🤫 <b>Послание:</b>\n📊 Аура: <b>${msg.aura}</b>\n\n"${msg.text}"\n\n<i>Автор скрыт за маской 🎭. Хочешь узнать, кто это?</i>`,
      { parse_mode: 'HTML', ...Markup.inlineKeyboard([
        [Markup.button.callback('🎭 Снять маску (149₽)', `buy_reveal_${msg.id}`)],
...
// === МАГАЗИН ===
bot.action('shop_reveal', (ctx) => { ctx.answerCbQuery(); ctx.reply(`🎭 <b>Снять маску</b>\nУзнай, кто скрывается за анонимкой!\n\n<b>149 ₽</b>`, { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.url('💳 Оплатить', generatePaymentLink(149, `${ctx.from.id}_reveal_0`))], [Markup.button.callback('✅ Я оплатил', 'check_reveal_0')]]) }); });
bot.action('shop_vip', (ctx) => { ctx.answerCbQuery(); ctx.reply(`💎 <b>VIP Статус</b>\n✅ 5 снятий масок\n✅ Значок 💎\n\n<b>399 ₽</b>`, { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.url('💳 Оплатить', generatePaymentLink(399, `${ctx.from.id}_vip`))], [Markup.button.callback('✅ Я оплатил', 'check_vip')]]) }); });
bot.action('shop_premium', (ctx) => { ctx.answerCbQuery(); ctx.reply(`👑 <b>PREMIUM — Всевидящее око</b>\n✅ Все маски сняты навсегда\n✅ Значок 👑\n\n<b>799 ₽</b>`, { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.url('💳 Оплатить', generatePaymentLink(799, `${ctx.from.id}_premium`))], [Markup.button.callback('✅ Я оплатил', 'check_premium')]]) }); });

bot.action(/^buy_reveal_(.+)$/, async (ctx) => { const id = ctx.match[1]; ctx.answerCbQuery(); ctx.reply(`🎭 <b>Снять маску</b> (149₽)`, { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.url('💳 Оплатить', generatePaymentLink(149, `${ctx.from.id}_reveal_${id}`))], [Markup.button.callback('✅ Я оплатил', `check_reveal_specific_${id}`)]) }); });

// === ОПЛАТЫ ===
bot.action('check_vip', async (ctx) => { await ctx.answerCbQuery('Проверяю...'); if (await checkYooMoneyPayment(`${ctx.from.id}_vip`)) { const u=db.users[ctx.from.id]; u.is_vip=true; u.vip_expiry=Date.now()+30*24*60*60*1000; u.reveal_credits=5; scheduleSave(); ctx.reply('💎 VIP активирован! У тебя 5 ключей для снятия масок.', {parse_mode:'HTML'}); } else ctx.reply('❌ Не найдено'); });
bot.action('check_premium', async (ctx) => { await ctx.answerCbQuery('Проверяю...'); if (await checkYooMoneyPayment(`${ctx.from.id}_premium`)) { const u=db.users[ctx.from.id]; u.is_premium=true; u.premium_expiry=Date.now()+30*24*60*60*1000; scheduleSave(); ctx.reply('👑 PREMIUM активирован! Ты видишь все лица без масок!', {parse_mode:'HTML'}); } else ctx.reply('❌ Не найдено'); });
bot.action('check_reveal_0', async (ctx) => { await ctx.answerCbQuery('Проверяю...'); if (await checkYooMoneyPayment(`${ctx.from.id}_reveal_0`)) { const m=Object.values(db.messages).find(m=>m.target_id===ctx.from.id&&!m.is_read); if(m){m.reveal_bought=true;scheduleSave(); ctx.replyWithPhoto(m.sender_photo, {caption: `🎭 <b>Маска снята:</b> ${m.sender_name}\n\n"${m.text}"`, parse_mode:'HTML'});} } else ctx.reply('❌ Не найдено'); });
bot.action(/^check_reveal_specific_(.+)$/, async (ctx) => { const id=ctx.match[1]; await ctx.answerCbQuery('Проверяю...'); if(await checkYooMoneyPayment(`${ctx.from.id}_reveal_${id}`)){ const m=getMessageById(parseInt(id)); if(m){m.reveal_bought=true;scheduleSave();ctx.replyWithPhoto(m.sender_photo, {caption: `🎭 <b>Маска снята:</b> ${m.sender_name}\n\n"${m.text}"`, parse_mode:'HTML'});} } else ctx.reply('❌ Не найдено'); });

// === АДМИН-ПАНЕЛЬ ===
bot.command('admin', async (ctx) => { if (ctx.from.id !== ADMIN_ID) return; ctx.reply('👑 <b>Админ-панель</b>', { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('📊 Статистика', 'admin_stats')], [Markup.button.callback('📢 Рассылка', 'admin_broadcast')], [Markup.button.callback('👑 Выдать Premium', 'admin_grant_premium')], [Markup.button.callback('🧹 Очистить базу', 'admin_clear_msgs')]]) }); });
bot.action('admin_stats', async (ctx) => { if (ctx.from.id !== ADMIN_ID) return; await ctx.answerCbQuery(); const u=Object.keys(db.users).length; const m=Object.keys(db.messages).length; const p=Object.values(db.users).filter(u=>u.is_premium&&u.premium_expiry>Date.now()).length; ctx.reply(`📊:\n👥 Юзеров: <b>${u}</b>\n👑 Premium: <b>${p}</b>\n📬 Сообщений: <b>${m}</b>`, {parse_mode:'HTML'}); });
bot.action('admin_broadcast', (ctx) => { if (ctx.from.id !== ADMIN_ID) return; ctx.answerCbQuery(); adminState.waitingForBroadcast=true; ctx.reply('📢 Отправьте текст:'); });
...
bot.action('admin_clear_msgs', (ctx) => { if (ctx.from.id !== ADMIN_ID) return; ctx.answerCbQuery(); const c=Object.keys(db.messages).length; db.messages={}; scheduleSave(); ctx.reply(`🧹 Удалено ${c}!`); });

// === ЗАПУСК ===
bot.launch().then(() => console.log('🤖 Бот Шепот (Маскарад) запущен!')).catch(err => console.error(err));
